...
import {
  ...
  USER_ACCEPT_PAYMENT_EVENT,
  USER_DISMISS_PAYMENT_EVENT,
...
let _acceptPromiseResolver: (value: any) => void = null;
let _acceptPromiseRejecter: (reason: any) => void = null;
let _userDismissSubscription: any = null;
let _userAcceptSubscription: any = null;
let _shippingAddressChangeSubscription: any = null;
let _shippingOptionChangeSubscription: any = null;
...
export default class PaymentRequest {
  ...
  _userAcceptPaymentFn: (PaymentResponse) => void; // function provided by user
  _userDismissPaymentFn: (Error) => void; // function provided by user
  ...
  _setupEventListeners() {
    // Internal Events
    _userDismissSubscription = DeviceEventEmitter.addListener(
      USER_DISMISS_EVENT,
      this._closePaymentRequest.bind(this)
    );
    _userAcceptSubscription = DeviceEventEmitter.addListener(
      USER_ACCEPT_EVENT,
    ...
    if (IS_IOS) {
      ...
      _shippingOptionChangeSubscription = DeviceEventEmitter.addListener(
        INTERNAL_SHIPPING_OPTION_CHANGE_EVENT,
        this._handleShippingOptionChange.bind(this)
      );

      // https://www.w3.org/TR/payment-request/#onshippingaddresschange-attribute
      _shippingAddressChangeSubscription = DeviceEventEmitter.addListener(
        INTERNAL_SHIPPING_ADDRESS_CHANGE_EVENT,
    ...
    const paymentResponse = new PaymentResponse({
      ...
      payerEmail:
        IS_ANDROID && this._options.requestPayerEmail
          ? details.payerEmail
          : null,
      billingAddress: this._options.requestBilling
          ? details.billingAddress
          : null,
    });
    if(this._userAcceptPaymentFn){
      return this._userAcceptPaymentFn(paymentResponse);
    }else{
      return _acceptPromiseResolver(paymentResponse);
    }
  }
  ...
  _closePaymentRequest() {
    ...
    if(this._userDismissPaymentFn){
      this._userDismissPaymentFn(new Error("AbortError"));
    }else{
      _acceptPromiseRejecter(new Error("AbortError"));
    }
  ...
  _removeEventListeners() {
    // Internal Events
    if(_userDismissSubscription){
      DeviceEventEmitter.removeSubscription(_userDismissSubscription);
    }
    if(_userAcceptSubscription){
      DeviceEventEmitter.removeSubscription(_userAcceptSubscription);
    }
    if (IS_IOS) {
      if(_shippingAddressChangeSubscription){
        DeviceEventEmitter.removeSubscription(
          _shippingAddressChangeSubscription
        );
      }
      if(_shippingOptionChangeSubscription){
        DeviceEventEmitter.removeSubscription(
  ...
  // https://www.w3.org/TR/payment-request/#onshippingoptionchange-attribute
  addEventListener(
    eventName: "shippingaddresschange" | "shippingoptionchange" | "useracceptpayment" | "userdismisspayment",
    fn: (e) => Promise<any>
  ) {
    ...
    if(eventName === USER_ACCEPT_PAYMENT_EVENT){
      return (this._userAcceptPaymentFn = fn.bind(this));
    }

    if(eventName === USER_DISMISS_PAYMENT_EVENT){
      return (this._userDismissPaymentFn = fn.bind(this));
    }
  }
  ...
  show(): Promise<PaymentResponseType> {
    this._acceptPromise = new Promise((resolve, reject) => {
      _acceptPromiseResolver = resolve;
      _acceptPromiseRejecter = reject;
    ...
    if(this._userAcceptPaymentFn && this._userDismissPaymentFn){
...
export const canMakePayments = (
  ...
  return NativePayments.canMakePayments(
    getPlatformMethodData(JSON.parse(methodData), Platform.OS)
  );
};
